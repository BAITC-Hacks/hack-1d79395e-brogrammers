...
A0–A6 и B0–B5 проверены на реальных выгрузках. Актуальный этап ядра — [STATUS_A](docs/STATUS_A.md), статус интерфейса — [STATUS_B](docs/STATUS_B.md).
Финальная сборка `v1.0` объединяет A0–A6 и B0–B5. Ниже — результаты чистого финального запуска. B6 проверит новый клон `main` после публикации; результат будет записан в [STATUS_B](docs/STATUS_B.md) и [forA.md](forA.md).
...
Реальная выгрузка `out`: 2248 узлов, 3119 рёбер, 107 кластеров, топ-30. Время из `run_meta.json`: 25.3703 с. Пропущенные этапы: [].
...
2. Выполните `python -m graf.check out` и `python -m pytest -q`. В проверенном окружении команды прошли все 49 тестов.
