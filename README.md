...
Опубликованный `v1.0` прошёл B6: отдельный чистый клон и новый venv, 49 тестов, пересчёт за 27,28 с, интерфейс без API-ключа. Актуальные статусы — [A](docs/STATUS_A.md) и [B](docs/STATUS_B.md).
После аудита в рабочих ветках улучшены объяснения A и исправлены граф, полнота XLSX и резервный денежный слой B. Ниже — числа свежих выгрузок A-005 (`2a70753`); интерфейс создаёт исправленный XLSX по кнопке. Пересоздание закоммиченного `out/evidence.xlsx` и общая финальная сборка выполняются A после получения B. История релиза `v1.0` сохранена.
...
Реальная выгрузка `out`: 2248 узлов, 3119 рёбер, 107 кластеров, топ-30. Время из `run_meta.json`: 27.1639 с. Пропущенные этапы: [].
<!-- ENDVALUE:FINAL_RESULTS_FROM_A4 -->
<!-- RESULTS_END -->

## 2. Быстрый старт

Для воспроизведения проверенного окружения используйте **Python 3.12** (проверен 3.12.14). Исходный requirements допускает также Python 3.11, но зафиксированный набор ниже проверен на 3.12. Из корня репозитория создайте окружение:
...
```bash
pip install -r requirements.txt -c tools/constraints-py312.txt && python run.py --data data --out out
```

Файл [tools/constraints-py312.txt](tools/constraints-py312.txt) фиксирует проверенные прямые и транзитивные зависимости. В PowerShell до версии 7 выполните две команды отдельно. Затем:
...
2. Выполните `python -m graf.check out` и `python -m pytest -q`. Последний результат проверок записан в [STATUS_B](docs/STATUS_B.md).
...
1. **Кого проверить:** выберите строку топ-листа или найдите любой gid в боковой панели.
2. **Почему:** в карточке сначала показаны основания с числами, контр-сигналы и «Что запросить дальше». Ниже — связи, точное правило роли и метрики с русскими подписями.
3. **Что наблюдается:** стрелки обозначают переводы; пунктир ego и контурные маркеры карты — неполные данные. На колене 4 отсутствие исходящих не означает удержание денег.
4. **Что передать на проверку:** во вкладке «Доказательства» подготовьте книгу выбранного узла либо всю базу. Общая книга включает все запросы данных, а не только запросы топ-30.

Приоритет учитывает наблюдаемый поток от seed и служит очередью проверки. Он не оптимизирует блокировку или разрушение сети; большой оборот из внешнего финансирования сам по себе не означает высокий приоритет этой модели.
...
Основной стек: Python 3.11/3.12, pandas и pyarrow для табличных данных, NetworkX для графа, Streamlit и Plotly для интерфейса, openpyxl для XLSX. Опциональный ассистент использует OpenAI SDK и Responses API; других внешних сервисов для работы проекта нет.
...
`pt = out_kzt / in_kzt`; для seed этот показатель не используется. Скоры ролей ограничены диапазоном 0–1; `pt` может превышать 1.
...
В текущей версии `graf.flow.chrono_reach` готов: ползунок Δ от 1 до 31 дня запускает кэшируемый пересчёт. Для более ранних выгрузок интерфейс использует подготовленные колонки 2 и 31 дней.
...
Листы XLSX: **Сводка**, **Признаки** (включая пороги), **Транзакции-основания**, **Пути денег** (по шагам), **Критерии** (точные ворота, формулы и порядок выбора), **Границы данных** (в общей книге — все 210 запросов текущей выгрузки; в книге узла — только его запросы).
...
Поиск по полному gid/префиксу и выбор строки топа ведут к карточке. Клик на карте сразу подсвечивает соседей и показывает кнопку открытия карточки. Скрытый фильтром выбранный узел не приглушает всю карту. Ego-граф ограничен ближайшими 250 узлами; неориентированная проекция используется только для раскладки, отображаемые связи остаются направленными. Карта рисует до 400 крупнейших рёбер, стрелки у 100. Ограничение отображения не меняет признаки карточки.
...
Тесты проверяют точность gid, направления, даты путей, поиск/навигацию, вкладки без ключа, листы XLSX и Responses API на моке. Добавлены регрессии видимости/выбора карты, отсутствия транзакций, полноты запросов и ссылок на узлы кластера. `smoke_ui.py` поднимает отдельный сервер на свободном localhost-порту на 15 секунд и всегда завершает его. B6 для v1.0 выполнен; живой API-вызов не нужен для основного сценария. Репетицию демо и отправку решения на платформу выполняет команда.
