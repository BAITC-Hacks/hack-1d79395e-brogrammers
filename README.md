...
Финальные результаты публикуются после A4. Значения `{{…}}` означают ожидаемый результат, а не ноль.
Скриншоты сняты с реальной папки `out/`; версия ядра указана в [описании снимков](docs/screenshots/README.md).

<!-- RESULTS_START -->
{{FINAL_RESULTS_FROM_A4}}
<!-- RESULTS_END -->
...
Пример evidence из финальной выгрузки: {{EVIDENCE_EXAMPLE_FROM_A4}}.
...
Компоненты публикуются как `prio_role`, `prio_money`, `prio_brokerage`, `prio_volume`, `prio_temporal`; итоговый множитель — `prio_multiplier`. Окончательная нормировка компонентов определяется [graf/priority.py](graf/priority.py) в A4.
...
Множители из конфигурации: нет хронологической связи 0.5; внешнее финансирование 0.7; seed 0.6, seed выше нижнего уровня 0.8; payer 0.2; исключённый счёт 0.0. Применение комбинаций проверяется по реализации A4, а не перемножается заново интерфейсом.
...
Аудит топ-10/топ-30 и калибровка: {{AUDIT_AND_TRUNCATION_FROM_A4}}.
...
Абляция и оседание по глубине: {{ABLATION_AND_TRACKED_FROM_A4}}.
...
Результат финального расчёта: {{RESILIENCE_FROM_A4}}.
...
Демо-узлы выбираются по [docs/node_review.md](docs/node_review.md), когда A завершит A5; в коде gid не зашиты. Пока файла нет, конкретные демо-примеры не утверждены.
