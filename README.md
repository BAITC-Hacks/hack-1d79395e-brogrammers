...
Получены реальные результаты A4/A5 (`a-core`, `02621e5`). Финальная проверка чистого main остаётся задачей B6 после сообщения A.
...
Реальная выгрузка `out`: 2248 узлов, 3119 рёбер, 107 кластеров, топ-30. Время из `run_meta.json`: 24.1422 с. Пропущенные этапы: [].
